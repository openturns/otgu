...
Scalar GeneralLinearModelAlgorithm::computeLogIntegratedLikelihoodPenalization() const
{
  ...
  switch (scalePrior_)
  {
    ...
    case JOINTLYROBUST:
    {
      const Scalar b0 = 1.0;//ResourceMap::GetAsScalar("GeneralLinearModelAlgorithm-JointlyRobustPriorB0");
      const Scalar b1 = 0.2;//ResourceMap::GetAsScalar("GeneralLinearModelAlgorithm-JointlyRobustPriorB1");
...
//       reducedCovarianceModelCopy.setScaleParametrization(ScaleParametrization::INVERSE);
      Point inverseScale(inputDimension);
      for (UnsignedInteger j = 0; j < inputDimension; ++ j)
        inverseScale[j] = 1.0 / scale[j];
...
//       }
    ...
    case REFERENCE:
    {
      ...
      for (UnsignedInteger k1 = 0; k1 < size; ++ k1)
      {
        ...
        {
          Matrix parameterGradient(reducedCovarianceModel_.parameterGradient(inputSample_[k1], inputSample_[k2]));
          for (UnsignedInteger j = 0; j < inputDimension; ++ j)
          {

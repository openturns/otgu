...
# Test 1
for prior in [otgu.GeneralLinearModelAlgorithm.NONE, otgu.GeneralLinearModelAlgorithm.JOINTLYROBUST, otgu.GeneralLinearModelAlgorithm.REFERENCE]:
    if True:#for parametrization in [ot.CovarianceModelImplementation.STANDARD, ot.CovarianceModelImplementation.INVERSE, ot.CovarianceModelImplementation.LOGINVERSE]:
        ...
        algo = otgu.KrigingAlgorithm(X, Y, covarianceModel, basis)
...
prior = otgu.GeneralLinearModelAlgorithm.JOINTLYROBUST
#parametrization = ot.CovarianceModelImplementation.STANDARD
covarianceModel = ot.MaternModel(scale, amplitude, nu)
#covarianceModel.setScaleParametrization(parametrization)
algo = otgu.KrigingAlgorithm(X, Y, covarianceModel, basis, False)
